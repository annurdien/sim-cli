...
static void swizzleInstance(Class cls, SEL orig, SEL repl) {
    ...
    if (!origMethod || !replMethod) {
        NSLog(@"[MiniCamInject] ⚠️  Cannot swizzle %@.%@", NSStringFromClass(cls), NSStringFromSelector(orig));
        return;
    }
...
static void swizzleClass(Class cls, SEL orig, SEL repl) {
    ...
    if (!origMethod || !replMethod) {
        NSLog(@"[MiniCamInject] ⚠️  Cannot swizzle class method %@.%@", NSStringFromClass(cls), NSStringFromSelector(orig));
        return;
    }
    method_exchangeImplementations(origMethod, replMethod);
}
...
- (void)msc_addInput:(AVCaptureInput *)input {
    NSLog(@"[MiniCamInject] AVCaptureSession addInput: silenced (no hardware)");
}
...
- (void)msc_setSampleBufferDelegate:(id<AVCaptureVideoDataOutputSampleBufferDelegate>)delegate
    ...
    NSLog(@"[MiniCamInject] captured delegate=%@ queue=%@", delegate, queue);
...
static void startDelivery(void) {
    ...
    NSLog(@"[MiniCamInject] delivery started @ %d fps", gFPS);
}
...
static void stopDelivery(void) {
    ...
    NSLog(@"[MiniCamInject] delivery stopped");
}
...
void MSCInstallHooks(SharedFrameReader* reader, int32_t fps) {
    ...
    NSLog(@"[MiniCamInject] hooks installed — fps=%d", fps);
}
